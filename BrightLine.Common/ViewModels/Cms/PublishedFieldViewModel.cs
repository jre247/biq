...
namespace BrightLine.Common.ViewModels.Models
{
	public class PublishModelInstanceFieldViewModel
	{
		...
		public PublishModelInstanceFieldViewModel(CmsField cmsField, List<string> valuesToSet, int? campaignId)
		{
			CmsFieldHelper = IoC.Resolve<ICmsFieldHelper>();
		...
		private void SetFieldValues(CmsField cmsField, List<string> valuesToSave, int? campaignId)
		{
			...
			else if (CmsFieldHelper.IsFieldResource(cmsField))
			{
