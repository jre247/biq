...
(function ($, undefined) {
	...
	var DEFAULTS = {
		...
		left: false,
		titleHeight: 40,	// the height of the title of it is not specified in css
		footHeight: 40,
	...
	MSGBOX.prototype = {
		...
		_completeOptions: function () {
			switch (this.options.type) {
				...
				case 'prompt':
					...
					if (this.options.showIcons && this.options.icons !== null && this.options.icons.length == 0)
			...
			if (!this.options.content && $.inArray(this.options.type, ['ajax', 'iframe', 'photo', 'image', 'album', 'gallery', 'custom']) == -1)
		...
		_assemble: function () {
			...
			this.$loaded = createElement('div', this.options.prefix + '-loaded', {
				...
				width: '100%',
				height: '100%',
				overflow: $.inArray(this.options.type, ['photo', 'image', 'album', 'gallery', 'custom']) == -1 ? 'auto' : 'hidden'
			});
