; (function () {
	...
	require.register("analytics/lib/integrations.js", function (exports, require, module) {
		...
		// Don't include mixpanel outside of UAT and PRO environments
		if (['uat', 'pro'].indexOf(_blenv) == -1) {
		  integrations.splice(integrations.indexOf('mixpanel'), 1)
		}
	...
	require.register("analytics/lib/integrations/amplitude.js", function (exports, require, module) {
		...
		Amplitude.prototype.initialize = function (options, ready) {
			(function (e, t) {
				...
				for (var c = 0; c < s.length; c++) { i(s[c]); } e.amplitude = r;
			})(window, document);
...
if (window.analytics) {
  window.analytics.initialize({ 'Google Analytics': 'UA-43175601-1', 'Mixpanel': { token: 'e92eaaf0cb0bd298b3c385cdccd53026', people: true } });
  if (_bl.user.IdentityName)
    window.analytics.identify(_bl.user.IdentityName);
  if (_bl.env == 'pro') {
    window.analytics.pageview(); analytics.track('Page View', {});
  }
} else {
  $.msgbox({
    id: "__adblock_checker_alert",
    buttons: ["Close"],
    buttonEvents: { "Close": function () { this.close(); } },
    title: "Ad Blocker",
    content: "You have an ad blocker running. To use all features of iQ, it needs to be disabled for the iq.brightline.tv domain.",
    type: "alert",
    fixed: true,
    resize: false,
    drag: false
